...
from credit_scoring.prepare.load import load_data
...
def preprocess_apps(apps):
    ...
    df_proc["PAYMENT_RATE"] = (
        df_proc["AMT_ANNUITY"] / df_proc["AMT_CREDIT"]
    )  # TODO dezdz
...
def select_top_variance_features(df: pd.DataFrame, n: int = 100):
    """
    Sélectionne les n colonnes de features avec la plus grande variance,
    tout en préservant impérativement ["TARGET", "SK_ID_CURR"].
    """
    # 1. Identifier les colonnes à protéger
    protected_cols = ["TARGET", "SK_ID_CURR"]

    # 2. Séparer les features du reste
    features_df = df.drop(
        columns=[c for c in protected_cols if c in df.columns], errors="ignore"
    )

    # 3. Calculer la variance sur les features uniquement
    variances = features_df.var()

    # 4. Sélectionner le top n
    top_features = variances.nlargest(n).index.tolist()

    # 5. Réassembler (on ajoute les colonnes protégées aux features sélectionnées)
    final_cols = [c for c in protected_cols if c in df.columns] + top_features

    return df[final_cols]


def select_important_features(df, type, n=20):
    if type == "ft":
        ft = pd.read_csv(
            "/Users/olivier/Developer/learning/credit-scoring/data/feature_selection/feature_importances.csv"
        )
        ft = ft.sort_values(by="absolute_importance", ascending=False)
        top_features = ft["feature"].head(n).tolist()
        return df[top_features]

    elif type == "shap":
        shap = pd.read_csv(
            "/Users/olivier/Developer/learning/credit-scoring/data/feature_selection/shap_importances.csv"
        )
        shap = shap.sort_values(by="mean_abs_shap", ascending=False)
        top_features = shap["feature"].head(n).tolist()
        return df[top_features]

    else:
        raise ValueError("type must be one of ['ft', 'shap']")
