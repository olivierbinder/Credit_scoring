...
from credit_scoring.data.preprocess import sanitize_feature_names, preprocess_apps
...
def _make_apps(n=10) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "DAYS_EMPLOYED":        [365243] * (n // 2) + list(rng.integers(-3000, -100, n // 2)),
...
def test_preprocess_apps_creates_ratio_columns():
    df = _make_apps(10)
    result = preprocess_apps(df)
    expected = ["DAYS_EMPLOYED_PERC", "INCOME_CREDIT_PERC",
